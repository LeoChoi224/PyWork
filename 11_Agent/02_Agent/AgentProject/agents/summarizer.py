...
class NewsSummarizerAgent:
    ...
    async def summarize_single_news(self, news_item: Dict[str, Any]) -> Dict[str, Any]:
        """단일 뉴스 요약 (오류 발생시 원본 내용 반환)"""
        content = news_item.get('content', "")

        try:
            # ④ 최소 콘텐츠 길이 검증으로 불필요한 API 호출 방지
            #   요약할 내용이 너무 짧거나 없는 경우, 굳이 비싼 LLM API를 호출하지 않도록 막는 
            #   가드레일 (guard rall) 역할을 합니다. 콘텐츠 길이가 50자 미만이면 의미 있는 요약이 어렵다고 
            #   판단하고, API 호출 비용을 절약하며 불필요한 작업을 방지합니다.
