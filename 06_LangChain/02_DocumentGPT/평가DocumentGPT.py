...
prompt = ChatPromptTemplate.from_messages([
    ...
    Context: {context}
    """),
    MessagesPlaceholder(variable_name='history'),  # 프롬프트에 내역도 같이 전달
    ("human", "{question}")
])
...
# invoke 함수 추가
def invoke_chain(retriever, question, history):
    chain = (
            {
...
if file:
    ...
    if message:
        ...
        with st.chat_message('ai'):
            invoke_chain(retriever, message, st.session_state["chat_history"])
